        ...
        if (l_BytesSent == -1)
        {
            ...
            if (errno == EAGAIN || errno == EWOULDBLOCK)
            {
                ...
                if (p_Network->m_TimeoutCounter >= GABLE_NET_TIMEOUT_TICKS)
                {
                    ...
                    GABLE_RequestInterrupt(p_Engine, GABLE_INT_NET);
                    return;
                }
            }
...
Bool GABLE_Connect (GABLE_Engine* p_Engine, const char* p_Address, Uint16 p_Port)
{
    ...
    // Point to the network context.
    GABLE_NetworkContext* l_Network = GABLE_GetNetwork(p_Engine);
    ...
    GABLE_CloseConnection(p_Engine);
    ...
    #if defined(GABLE_LINUX)
    {
        ...
        while (l_Current != NULL)
        {
            ...
            l_Network->m_Socket = socket(
                l_Current->ai_family,
                l_Current->ai_socktype,
                l_Current->ai_protocol
            );
            if (l_Network->m_Socket != GABLE_INVALID_SOCKET)
            {
                break;
            }
        ...
        if (connect(l_Network->m_Socket, (struct sockaddr*) &l_RemoteAddr, sizeof(l_RemoteAddr)) == -1)
        {
            GABLE_perror("Failed to connect to '%s:%u'", p_Address, p_Port);
            close(l_Network->m_Socket);
            l_Network->m_Socket = GABLE_INVALID_SOCKET;
            freeaddrinfo(l_AddrInfo);
            return false;
        }
