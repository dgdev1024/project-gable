 ...
 */
typedef void (*GABLE_AudioMixCallback) (const GABLE_AudioSample*);
...
/**
 * @brief      Reads a nibble from the APU's wave pattern RAM.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Index   The index in the wave pattern RAM to read from.
 * 
 * @return     The nibble read from the wave pattern RAM.
 */
...
/**
 * @brief      Writes a nibble to the APU's wave pattern RAM.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Index   The index in the wave pattern RAM to write to.
 * @param      p_Value   The nibble to write to the wave pattern RAM.
 */
void GABLE_WriteWaveNibble (GABLE_APU* p_APU, Uint8 p_Index, Uint8 p_Value);

/**
 * @brief      Writes a 32-character wave string to the APU's wave pattern RAM.
 * 
 * The wave string provided must be a 32-character hexadecimal string, where each character
 * represents a nibble (4 bits) of a sample in the wave pattern RAM. The wave string must be
 * exactly 32 characters long, and each character must be a valid hexadecimal digit (`0`-`9`, `A`-`F`).
 * 
 * @param      p_APU         A pointer to the GABLE Engine APU instance.
 * @param      p_WaveString  The 32-character wave string to write to the wave pattern RAM.
 * 
 * @return     `true` if the wave string provided was valid and written to the wave pattern RAM;
 *             `false` otherwise.
 */
...
/**
 * @brief      Enables or disables the GABLE Engine's APU.
 * 
 * If the APU is disabled, then all of its hardware registers are reset and made read-only, except
 * for the audio master control register (`NR52`).
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Enable  `true` to enable the APU; `false` to disable the APU.
 */
void GABLE_SetAudioMasterEnable (GABLE_APU* p_APU, Bool p_Enable);

/**
 * @brief      Sets panning settings for the given audio channel.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Channel The audio channel to set the panning settings for.
 * @param      p_Left    `true` to output the channel to the left speaker; `false` otherwise.
 * @param      p_Right   `true` to output the channel to the right speaker; `false` otherwise.
 */
void GABLE_SetAudioPanning (GABLE_APU* p_APU, GABLE_AudioChannel p_Channel, Bool p_Left, Bool p_Right);

/**
 * @brief      Sets the master volume for the left and right speakers.
 * 
 * @param      p_APU    A pointer to the GABLE Engine APU instance.
 * @param      p_Left   The volume level for the left speaker, from 0 (mute) to 7 (max volume).
 * @param      p_Right  The volume level for the right speaker, from 0 (mute) to 7 (max volume).
 */
void GABLE_SetMasterVolume (GABLE_APU* p_APU, Uint8 p_Left, Uint8 p_Right);

/**
 * @brief      Sets the frequency sweep settings for the `PC1` audio channel.
 * 
 * @param      p_APU       A pointer to the GABLE Engine APU instance.
 * @param      p_Step      The individual step value for the frequency sweep unit.
 * @param      p_Direction The direction of the frequency sweep.
 * @param      p_Pace      The sweep pace value for the frequency sweep unit.
 */
void GABLE_SetPulseFrequencySweep (GABLE_APU* p_APU, Uint8 p_Step, GABLE_FrequencySweepDirection p_Direction, Uint8 p_Pace);

/**
 * @brief      Sets the duty cycle pattern of the given pulse channel.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Channel The pulse channel to set the duty cycle pattern for.
 * @param      p_Duty    The duty cycle pattern to set for the channel.
 */
void GABLE_SetPulseDutyCycle (GABLE_APU* p_APU, GABLE_AudioChannel p_Channel, GABLE_PulseDutyCycle p_Duty);

/**
 * @brief      Sets the length timer settings for the given audio channel.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Channel The audio channel to set the length timer settings for.
 * @param      p_Length  The initial length timer value for the channel.
 */
void GABLE_SetAudioLengthTimer (GABLE_APU* p_APU, GABLE_AudioChannel p_Channel, Uint8 p_Length);

/**
 * @brief      Sets the initial volume for the given audio channel.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Channel The audio channel to set the initial volume for.
 * @param      p_Volume  The initial volume level for the channel, from 0 (mute) to 15 (max volume).
 */
void GABLE_SetAudioInitialVolume (GABLE_APU* p_APU, GABLE_AudioChannel p_Channel, Uint8 p_Volume);

/**
 * @brief      Sets the envelope sweep settings for the given audio channel.
 * 
 * @param      p_APU       A pointer to the GABLE Engine APU instance.
 * @param      p_Channel   The audio channel to set the envelope sweep settings for.
 * @param      p_Pace      The sweep pace value for the envelope sweep unit.
 * @param      p_Direction The direction of the envelope sweep.
 */
...
/**
 * @brief      Sets the period value for the given audio channel.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Channel The audio channel to set the period value for.
 * @param      p_Period  The period value to set for the channel.
 */
void GABLE_SetAudioPeriod (GABLE_APU* p_APU, GABLE_AudioChannel p_Channel, Uint16 p_Period);

/**
 * @brief      Enables or disables the length timer for the given audio channel.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Channel The audio channel to enable or disable the length timer for.
 * @param      p_Enable  `true` to enable the length timer; `false` to disable the length timer.
 */
void GABLE_SetAudioLengthTimerEnable (GABLE_APU* p_APU, GABLE_AudioChannel p_Channel, Bool p_Enable);

/**
 * @brief      Enables or disables the digital-to-analog converter (DAC) for the wave channel.
 * 
 * Turning off a channel's DAC also disables the channel.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Enable  `true` to enable the wave channel's DAC; `false` to disable the wave channel's DAC.
 */
void GABLE_SetWaveDACEnable (GABLE_APU* p_APU, Bool p_Enable);

/**
 * @brief      Sets the output level for the wave channel.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Level   The output level to set for the wave channel.
 */
void GABLE_SetWaveOutputLevel (GABLE_APU* p_APU, GABLE_WaveOutputLevel p_Level);

/**
 * @brief      Sets the clock divider, LFSR width, and clock shift settings for the noise channel.
 * 
 * @param      p_APU       A pointer to the GABLE Engine APU instance.
 * @param      p_Divider   The clock divider setting for the noise channel.
 * @param      p_Width     The LFSR width setting for the noise channel.
 * @param      p_Shift     The clock shift setting for the noise channel.
 */
void GABLE_SetNoiseLFSRControl (GABLE_APU* p_APU, Uint8 p_Divider, Uint8 p_Width, Uint8 p_Shift);

/**
 * @brief      Triggers the given audio channel to generate sound.
 * 
 * @param      p_APU     A pointer to the GABLE Engine APU instance.
 * @param      p_Channel The audio channel to trigger.
 */
